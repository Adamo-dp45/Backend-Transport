...
- **Les modules**
    ...
    > Le module `Système` (super admin) : Maintenance, Corbeille
        ...
        > CORBEILLE multi-entreprises (`Corbeille` ressource virtuelle non persistée, `CorbeilleService` + `CorbeilleRegistry`) : le super admin (HORS périmètre entreprise) voit et gère les enregistrements SOFT-DELETÉS (`deletedAt IS NOT NULL`) de TOUTES les compagnies. Le super admin échappant à `EntrepriseScopeExtension` (ni filtre `identreprise` ni `deletedAt IS NULL`), la corbeille requête en direct (QueryBuilder) SANS scope ; le service ne dépend JAMAIS de l'entreprise de l'ACTEUR (le super admin n'en a pas — piège du provider-avant-security évité, cf. `/api/me`), l'entreprise affichée est celle de CHAQUE élément (`getIdentreprise` → libellé via `EntrepriseRepository`)
